...
pub fn serve(socket_path: &Path, vk: VulkanContext) -> std::io::Result<()> {
    ...
    for stream in listener.incoming() {
        match stream {
            Ok(stream) => handle_connection(stream, &vk, &registry),
...
fn handle_connection(stream: UnixStream, vk: &VulkanContext, registry: &Mutex<Registry>) {
    ...
    for line in reader.lines() {
        ...
        let result = match serde_json::from_str::<Proposal>(&line) {
            Ok(proposal) => dispatch(proposal, vk, registry),
            Err(e) => {
...
fn dispatch(proposal: Proposal, vk: &VulkanContext, registry: &Mutex<Registry>) -> ProposalResult {
    ...
    match proposal {
        ...
        Proposal::AddArtifact { artifact_id, description, source_uri, .. } => {
            reg.add_artifact(&artifact_id, &description, source_uri.as_deref());
            ProposalResult::ok(&proposal_id, Some(json!({ "artifactId": artifact_id })))
        }
        Proposal::RetireArtifact { artifact_id, .. } => match reg.retire_artifact(&artifact_id) {
            Ok(()) => ProposalResult::ok(&proposal_id, None),
            Err(e) => ProposalResult::err(&proposal_id, e),
        },
    }
